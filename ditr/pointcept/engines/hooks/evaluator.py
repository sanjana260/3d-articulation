...
@HOOKS.register_module()
class ArticEvaluator(HookBase):
    """Multi-task evaluator for Articulate3D: segmentation mIoU + articulation metrics."""
    ...
    def eval(self):
        ...
        for i, input_dict in enumerate(self.trainer.val_loader):
            ...
            if "type_logits" in output_dict:
                type_pred = output_dict["type_logits"].max(1)[1]
                type_gt = input_dict["artic_type"]
                type_correct = (type_pred == type_gt).sum().item()
                type_total = type_gt.numel()
                self.trainer.storage.put_scalar("val_type_correct", type_correct)
                self.trainer.storage.put_scalar("val_type_total", type_total)

                # Axis error (rotation parts only)
                rot_mask = type_gt == 1
                if rot_mask.sum() > 0:
                    axis_pred_r = F.normalize(output_dict["axis_pred"][rot_mask], dim=-1)
                    axis_gt_r = F.normalize(input_dict["artic_axis"][rot_mask], dim=-1)
                    cos_sim = F.cosine_similarity(axis_pred_r, axis_gt_r, dim=-1)
                    angle_err = torch.acos(cos_sim.clamp(-1, 1)) * (180 / 3.14159265)
                    self.trainer.storage.put_scalar(
                        "val_axis_error_rot", angle_err.cpu().numpy()
                    )

                # Origin error
                if rot_mask.sum() > 0:
                    o_pred_r = output_dict["origin_pred"][rot_mask]
                    o_gt_r = input_dict["artic_origin"][rot_mask]
                    a_gt_r = input_dict["artic_axis"][rot_mask]
                    diff = o_pred_r - o_gt_r
                    cross = torch.cross(diff, a_gt_r, dim=-1)
                    dist_to_line = cross.norm(dim=-1) / (a_gt_r.norm(dim=-1) + 1e-8)
                    self.trainer.storage.put_scalar(
                        "val_origin_error_rot", dist_to_line.cpu().numpy()
                    )

                trans_mask = type_gt == 2
                if trans_mask.sum() > 0:
                    o_pred_t = output_dict["origin_pred"][trans_mask]
                    o_gt_t = input_dict["artic_origin"][trans_mask]
                    origin_err_t = (o_pred_t - o_gt_t).norm(dim=-1)
                    self.trainer.storage.put_scalar(
                        "val_origin_error_trans", origin_err_t.cpu().numpy()
                    )

                # Range error
                artic_mask = type_gt > 0
                if artic_mask.sum() > 0:
                    range_err = (
                        output_dict["range_pred"][artic_mask]
                        - input_dict["artic_range"][artic_mask]
                    ).abs().mean(dim=-1)
                    self.trainer.storage.put_scalar(
                        "val_range_error", range_err.cpu().numpy()
                    )
        ...
        if self.trainer.storage.history("val_type_correct") is not None:
            type_correct_total = self.trainer.storage.history("val_type_correct").total
            type_total_total = self.trainer.storage.history("val_type_total").total
            type_acc = type_correct_total / (type_total_total + 1e-10)

            axis_err_hist = self.trainer.storage.history("val_axis_error_rot")
            if axis_err_hist is not None and len(axis_err_hist._values) > 0:
                all_axis_err = np.concatenate(
                    [v for v in axis_err_hist._values if isinstance(v, np.ndarray)]
                )
                mean_axis_err = np.mean(all_axis_err) if len(all_axis_err) > 0 else 0.0

            origin_err_r_hist = self.trainer.storage.history("val_origin_error_rot")
            origin_err_t_hist = self.trainer.storage.history("val_origin_error_trans")
            all_origin_err = []
            if origin_err_r_hist is not None:
                for v in origin_err_r_hist._values:
                    if isinstance(v, np.ndarray) and len(v) > 0:
                        all_origin_err.append(v)
            if origin_err_t_hist is not None:
                for v in origin_err_t_hist._values:
                    if isinstance(v, np.ndarray) and len(v) > 0:
                        all_origin_err.append(v)
            if all_origin_err:
                all_origin_err = np.concatenate(all_origin_err)
                mean_origin_err = np.mean(all_origin_err) if len(all_origin_err) > 0 else 0.0

            range_err_hist = self.trainer.storage.history("val_range_error")
            if range_err_hist is not None and len(range_err_hist._values) > 0:
                all_range_err = np.concatenate(
                    [v for v in range_err_hist._values if isinstance(v, np.ndarray)]
                )
                mean_range_err = np.mean(all_range_err) if len(all_range_err) > 0 else 0.0

        self.trainer.logger.info(
            "Val result: mIoU {:.4f} | TypeAcc {:.4f} | AxisErr {:.2f}deg | "
            "OriginErr {:.4f} | RangeErr {:.4f}".format(
                m_iou, type_acc, mean_axis_err, mean_origin_err, mean_range_err
            )
        )
        ...
        if self.trainer.writer is not None:
            self.trainer.writer.add_scalar("val/loss", loss_avg, current_epoch)
            self.trainer.writer.add_scalar("val/mIoU", m_iou, current_epoch)
            self.trainer.writer.add_scalar("val/type_acc", type_acc, current_epoch)
            self.trainer.writer.add_scalar("val/axis_err", mean_axis_err, current_epoch)
            self.trainer.writer.add_scalar("val/origin_err", mean_origin_err, current_epoch)
            self.trainer.writer.add_scalar("val/range_err", mean_range_err, current_epoch)
