...
def parse_scene(
    ...
    # skip scenes that are not downloaded
    if not mesh_path.exists():
        print(f"Skipping {name}: mesh not found at {mesh_path}")
        return
    if split != "test" and not segs_path.exists():
        print(f"Skipping {name}: segments.json not found at {segs_path}")
        return
...
def filter_map_classes(mapping, count_thresh, count_type, mapping_type):
    if count_type in mapping.columns:
